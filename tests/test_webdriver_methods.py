...
class Configure():
    ...
    def driver_get_text(self, url):
        ...
        driver.get(url)
        sleep(2)
        h2 = driver.find_element(By.XPATH, value="//h2[contains(., 'How We Select the Best Online Casinos')]")
        h2_text = h2.text
        sleep(2)
        return h2_text
...
def test_compare_url():
    assert conf.driver_current_url(url) == "slotozilla.com"


def test_hq_text():
    assert conf.driver_get_text(url) == "testettts"



def test_href():
    assert conf.driver_get_tag(url) == "test"
